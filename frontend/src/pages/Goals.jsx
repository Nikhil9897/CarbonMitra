  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Create Goal Form */}
        <div className="lg:col-span-1 bg-white border border-borderEco p-6 rounded-3xl h-fit shadow-sm">
          <h2 className="text-xl font-extrabold mb-6 flex items-center space-x-2 text-slate-800">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border bg-bgEco border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 text-textEco ${errors.targetReduction ? 'border-red-500' : ''}`}
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border bg-bgEco border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 text-textEco ${errors.periodDays ? 'border-red-500' : ''}`}
              />
              {errors.periodDays && (
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border bg-bgEco border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 text-textEco ${errors.startDate ? 'border-red-500' : ''}`}
              />
              {errors.startDate && (
        ...
        <div className="lg:col-span-2 space-y-6">
          {loading ? (
            <div className="bg-white border border-borderEco p-8 rounded-3xl flex items-center justify-center shadow-sm">
            ...
            goals.map((goal) => {
              ...
              return (
                <div key={goal.id} className="bg-white border border-borderEco p-6 rounded-3xl relative overflow-hidden shadow-sm hover:shadow-md transition">
                  {/* Goal Header */}
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center space-x-3">
                      <div className={`p-2.5 rounded-xl ${goal.status === 'ACTIVE' ? 'bg-primary-50 text-primary-600 border border-primary-100' : goal.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                        <Target size={20} />
                      </div>
                      <div>
                    ...
                    <div className="space-y-4 mt-2">
                      ...
                      <div>
                        <div className="flex justify-between text-xs font-bold mb-1">
                          ...
                          <span className="text-primary-600">{progress.percentComplete.toFixed(1)}%</span>
                        </div>
                      ...
                      {/* Stats Metrics Grid */}
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-2 border-t border-b border-slate-100 text-xs font-semibold">
                        <div>
                          <span className="text-slate-400 block">Baseline Emissions</span>
                          <span className="font-extrabold text-slate-800 mt-0.5 block">{progress.baselineEmissions.toFixed(1)} kg</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block">Actual Emissions</span>
                          <span className="font-extrabold text-slate-800 mt-0.5 block">{progress.actualEmissions.toFixed(1)} kg</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block">Total Carbon Saved</span>
                          <span className="font-extrabold text-primary-600 mt-0.5 block">{progress.carbonSaved.toFixed(1)} kg</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block">Days Remaining</span>
                          <span className="font-extrabold text-slate-800 mt-0.5 block">{progress.daysRemaining} days</span>
                        </div>
                      </div>

                      {/* Completion Prediction Insights */}
                      {prediction && (
                        <div className="p-3 bg-slate-50 border border-slate-100 rounded-2xl flex items-start space-x-3 text-xs leading-relaxed font-semibold">
                          {prediction.onTrack ? (
                  ...
                  {goal.status === 'COMPLETED' && (
                    <div className="mt-2 text-xs flex items-center space-x-2 text-emerald-650 bg-emerald-50 border border-emerald-100 p-3 rounded-xl font-bold">
                      <Sparkles size={16} />
                  ...
                  {goal.status === 'FAILED' && (
                    <div className="mt-2 text-xs flex items-center space-x-2 text-red-700 bg-red-50 border border-red-100 p-3 rounded-xl font-bold">
                      <AlertCircle size={16} />
            ...
            <div className="bg-white border border-borderEco p-12 rounded-3xl text-center text-slate-400 shadow-sm">
              <Target size={36} className="mx-auto text-slate-300 mb-2" />
              <p className="font-extrabold text-slate-500">No goals set yet</p>
