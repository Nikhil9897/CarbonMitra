          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border bg-bgEco border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 text-textEco ${errors.quantity ? 'border-red-500' : ''}`}
              />
              {errors.quantity && (
        ...
        <div className="lg:col-span-2 bg-white border border-borderEco p-6 rounded-3xl shadow-sm">
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-sm">
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-emerald-50/20 transition">
                      <td className="py-3.5 text-slate-500">
                        {new Date(log.logDate).toLocaleDateString()}
                      </td>
                      <td className="py-3.5">
                        <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider
                          ${log.category === 'TRANSPORT' ? 'bg-blue-50 text-blue-600' :
                            log.category === 'ELECTRICITY' ? 'bg-amber-50 text-amber-600' :
                            log.category === 'FOOD' ? 'bg-emerald-50 text-emerald-600' :
            ...
            <div className="text-center py-16 text-slate-400">
              ...
              <p className="font-bold text-slate-500">No carbon logs yet</p>
