  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400">Manage user authorization, review corporate organizations, and update dynamic CO₂ conversion factors.</p>
      </div>

      {/* Tabs list header */}
      <div className="flex border-b border-slate-200 dark:border-dark-800 space-x-6 text-sm font-semibold">
        <button
          onClick={() => setActiveTab('users')}
          className={`pb-3 flex items-center space-x-2 transition ${activeTab === 'users' ? 'border-b-2 border-primary-500 text-primary-600 dark:text-primary-400' : 'text-slate-500 dark:text-slate-400'}`}
        >
          <Users size={16} />
          <span>Users</span>
        </button>
        <button
          ...
          className={`pb-3 flex items-center space-x-2 transition ${activeTab === 'orgs' ? 'border-b-2 border-primary-500 text-primary-600 dark:text-primary-400' : 'text-slate-500 dark:text-slate-400'}`}
        >
          <Building2 size={16} />
        ...
        <button
          ...
          className={`pb-3 flex items-center space-x-2 transition ${activeTab === 'factors' ? 'border-b-2 border-primary-500 text-primary-600 dark:text-primary-400' : 'text-slate-500 dark:text-slate-400'}`}
        >
          <Flame size={16} />
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-dark-800 text-sm">
                  {users.map(u => (
                    <tr key={u.id} className="hover:bg-slate-50/50 dark:hover:bg-dark-800/20 transition">
                      <td className="py-3.5 px-6 font-semibold text-slate-800 dark:text-slate-200">{u.username}</td>
                      <td className="py-3.5 px-6 text-slate-500 dark:text-slate-400">{u.email}</td>
                      <td className="py-3.5 px-6">
                        <span className="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-100 dark:bg-dark-800 text-slate-600 dark:text-slate-400">
                          {u.role.replace('ROLE_', '')}
                        </span>
                      </td>
                      <td className="py-3.5 px-6 text-slate-500">{u.provider}</td>
          ...
          {activeTab === 'orgs' && (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-dark-800 text-sm">
                  {organizations.map(o => (
                    <tr key={o.id} className="hover:bg-slate-50/50 dark:hover:bg-dark-800/20 transition">
                      <td className="py-4 px-6 font-bold text-slate-400">{o.id}</td>
                      <td className="py-4 px-6 font-semibold text-slate-800 dark:text-slate-200">{o.name}</td>
                      <td className="py-4 px-6 text-slate-500 dark:text-slate-400">{o.adminUser?.username || 'Unassigned'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {activeTab === 'factors' && (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-dark-800 text-sm">
                  {factors.map(f => (
                    <tr key={f.id} className="hover:bg-slate-50/50 dark:hover:bg-dark-800/20 transition">
                      <td className="py-3.5 px-6 font-semibold text-slate-800 dark:text-slate-200">{f.activityType.replaceAll('_', ' ')}</td>
                      <td className="py-3.5 px-6 text-slate-400">{f.unit}</td>
                      <td className="py-3.5 px-6 font-bold text-slate-900 dark:text-slate-100">{f.kgCo2ePerUnit}</td>
                      <td className="py-3.5 px-6 text-slate-500 dark:text-slate-400">{f.source}</td>
                      <td className="py-3.5 px-6 text-center">
                        <button
      ...
      {editingFactor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white dark:bg-dark-900 border border-slate-100 dark:border-dark-800 rounded-3xl p-6 shadow-xl">
            ...
            <div className="space-y-4">
              ...
              <div className="flex space-x-3 pt-2">
                <button
                  ...
                  className="w-1/2 border border-slate-200 dark:border-dark-700 hover:bg-slate-50 text-slate-500 py-2.5 rounded-xl text-sm font-semibold"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSaveFactor}
                  className="w-1/2 bg-primary-600 hover:bg-primary-700 text-white py-2.5 rounded-xl text-sm font-semibold shadow-lg shadow-primary-600/10 flex items-center justify-center space-x-2"
                >
                  <Save size={16} />
