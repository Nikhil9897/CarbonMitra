...
const Profile = () => {
  ...
  if (earnedBadgesCount >= 6) {
    ...
    rankColor = 'text-emerald-700 bg-emerald-50 border border-emerald-100';
  } else if (earnedBadgesCount >= 3) {
    rankName = 'Planet Advocate 🌱';
    rankColor = 'text-primary-700 bg-primary-50 border border-primary-100';
  }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* LEFT COLUMN: ID Card & Edit profile details */}
        <div className="lg:col-span-1 space-y-6">
          
          {/* GREEN ID CARD */}
          <div className="relative overflow-hidden bg-gradient-to-br from-emerald-600 to-teal-800 border border-emerald-500/30 text-white rounded-3xl shadow-xl shadow-emerald-500/10 p-6 flex flex-col justify-between aspect-[1.586/1]">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="flex flex-col items-center py-2">
                <div className="relative group">
                  <div className="w-24 h-24 rounded-full border-4 border-slate-100 overflow-hidden bg-slate-50 flex items-center justify-center relative shadow-sm">
                    {profilePic ? (
                ...
                <span className="text-[10px] text-slate-400 mt-2 font-semibold">Upload image or use camera</span>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2.5 rounded-xl border bg-bgEco border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 text-textEco ${errors.username ? 'border-red-500' : ''}`}
                />
                {errors.username && (
              ...
              <div>
                ...
                <input
                  ...
                  disabled
                  value={user?.email || ''}
                  className="w-full px-4 py-2.5 rounded-xl border bg-slate-100 border-slate-200/60 text-sm text-slate-400 cursor-not-allowed outline-none"
                />
              </div>

              <div className="p-3 bg-red-50 border border-red-100 rounded-2xl flex items-start space-x-2 text-[11px] text-red-700 leading-relaxed font-semibold">
              ...
              <div className="pt-2">
                <div className="flex justify-between items-center text-xs text-slate-400 font-bold uppercase tracking-wider pb-2 border-b border-slate-100">
                ...
                <div className="mt-3 space-y-2 text-xs font-semibold">
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-700">{user?.provider}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">System Role</span>
                    <span className="text-slate-700">{user?.role?.replace('ROLE_', '')}</span>
                  </div>
        ...
        <div className="lg:col-span-2 bg-white border border-borderEco p-6 rounded-3xl shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-extrabold flex items-center space-x-2 text-slate-800">
              ...
              {badges.map((badge) => (
                <div 
                  ...
                  className={`flex flex-col items-center text-center p-5 rounded-3xl border transition duration-300
                    ${badge.awarded 
                      ? 'bg-gradient-to-b from-primary-50/50 to-white border-primary-100 shadow-sm shadow-primary-500/5' 
                  ...
                  <div className="relative">
                    ...
                    {!badge.awarded && (
                      <div className="absolute inset-0 flex items-center justify-center text-slate-500">
                        <Key size={18} className="bg-slate-200 p-0.5 rounded-full" />
                      </div>
                    )}
                  </div>

                  <h3 className="font-extrabold text-slate-850 mt-4">{badge.name}</h3>
                  <p className="text-xs text-slate-500 mt-1 max-w-[170px] min-h-[32px] leading-relaxed font-semibold">
                    {badge.description}
                  </p>

                  {badge.awarded ? (
                    <span className="text-[10px] font-bold text-primary-600 mt-3 flex items-center">
        ...
        {cameraActive && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white border border-borderEco rounded-3xl p-6 w-full max-w-md shadow-2xl relative">
              <button
                onClick={closeCamera}
              ...
              <div className="relative aspect-square w-full rounded-2xl bg-black overflow-hidden flex items-center justify-center border border-slate-100">
                <video
                  ref={videoRef}
                  autoPlay
                  playsInline
              ...
              <div className="mt-6 flex space-x-3">
                <button
                  ...
                  className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl transition duration-150 shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Capture Photo
                </button>
                <button
                  type="button"
                  onClick={closeCamera}
                  className="px-4 border border-slate-200 text-slate-700 font-bold py-2.5 rounded-xl hover:bg-slate-50 transition duration-150"
                >
                  Cancel
                </button>
              </div>
